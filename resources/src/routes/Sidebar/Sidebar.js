  ...
  render() {
    ...
    this.state.usermenuList.map((ite,i) =>{ 
        if(this.state.usermenuList[i].userchildMenuList!=false){
          userchildmenu.push(this.state.usermenuList[i].userchildMenuList.map((item,index) =>
          <Menu.Item key={item.uuid}>
          {(item.is_system_menu===1)?(<a title={item.name} href={`${item.redirect_url}?key=${item.uuid}&openKeys=${item.parent_uuid}`}>{(item.icon_url!=="")?(<div><img src={item.icon_url} style={{width:14,height:14,marginRight:10}} alt=""/>{item.name}</div>):(<div style={{marginLeft:24}}>{item.name}</div>)}</a>):
         <a title={item.name} href={`${item.redirect_url}?access_token=${this.state.access_token}&key=${item.uuid}&openKeys=${item.parent_uuid}`}>{(item.icon_url!=="")?(<div><img src={item.icon_url} style={{width:14,height:14,marginRight:10}} alt=""/>{item.name}</div>):(<div style={{marginLeft:24}}>{item.name}</div>)}</a>}
         </Menu.Item>
          ));
        }else{
          userchildmenu.push([]);
